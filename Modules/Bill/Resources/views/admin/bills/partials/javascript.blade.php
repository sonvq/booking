<script>
    ...
    $(document).ready(function() {
        setTimeout(function(){
            ...
            @if(!$editMode)
                $('select.selectize-status')[0].selectize.setValue('{{\Modules\Bill\Entities\Bill::STATUS_PENDING}}');
                $('select.selectize-status')[0].selectize.lock();
            @else
                @if($canChangeStatus && $user->id != $bill->author_id && $bill->status === \Modules\Bill\Entities\Bill::STATUS_PENDING)
                    $('select.selectize-status')[0].selectize.unlock();
                @else
                    $('select.selectize-status')[0].selectize.lock();
                @endif

                @if($bill->status === \Modules\Bill\Entities\Bill::STATUS_CONFIRMED)
